class Aventura {
  ...
  cargarPantallas(lineas) {
    ...
    this.pantallas[13] = new PantallaUnBoton(imagenesJuego[1], 300, 50, 14, "Siguiente");
    //----JUEGO ---//
    this.pantallas[14] = new Juego(15, 16);
    this.pantallas[15] = new PantallaUnBoton(imagenesJuego[2], 300, 50, 14, "Siguiente");//ganar
    this.pantallas[16] = new PantallaUnBoton(imagenesJuego[3], 300, 50, 14, "Siguiente");//perder
    //final despues de perder
    /*this.pantallas[12] = new PantallaUnBoton(imgs[8], 300, 360,"Siguiente", this.lineasTexto);//sigue por aca si gana
     this.pantallas[13] = new PantallaUnBoton(imgs[9], 300, 360,"Siguiente", this.lineasTexto);
     this.pantallas[14] = new PantallaUnBoton(imgs[10], 300, 360,"Siguiente", this.lineasTexto);// Final
     */
  }

  click(btn) {
    this.pantallas[this.verPantalla].click(btn)
      this.verPantalla = this.pantallas[this.verPantalla].click(btn)
      console.log(this.pantallas)
  }
  //this.verPantalla = this.pantallas[this.verPantalla].controlarGanaroPerder() retorna la pantalla siguiente si ganaste
  pantallaGanoOPerdio() {
    this.verPantalla = this.pantallas[this.verPantalla].controlarGanaroPerder()
  }

  btnApretado(btn) {
    this.pantallas[14].btnApretado(btn);
  }

  btnTipeado(btn) {
    this.pantallas[14].btnTipeado(btn);
  }
}
